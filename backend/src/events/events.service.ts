...
// import { Event, Session, SessionCategory, SessionStatus } from './types';
...
//   private readonly openF1BaseUrl = 'https://api.openf1.org/v1';
...
//     const response = await axios.get<OpenF1Meeting[]>(
//       `${this.openF1BaseUrl}/meetings`,
//       {
//         params: { year },
//       },
//     );
...
//     const response = await axios.get<OpenF1Session[]>(
//       `${this.openF1BaseUrl}/sessions`,
//       {
...
//     // For now, search inside the selected dataset year-by-year fallback is not added.
//     // This keeps the implementation simple for the current step.
...
//     const response = await axios.get<OpenF1Driver[]>(
//       `${this.openF1BaseUrl}/drivers`,
//       {
//         params: { session_key: sessionKey },
//       },
//     );
...
//     const response = await axios.get<OpenF1Lap[]>(
//       `${this.openF1BaseUrl}/laps`,
//       {
//         params: { session_key: sessionKey },
//       },
//     );
...
//       (item) => item.meeting_key === targetSession.meeting_key,
//     );

//     if (!meeting) {
//       throw new NotFoundException(
//         `Meeting for session "${targetSession.session_key}" not found`,
//       );
//     }
...
//       meeting.meeting_key,
//     );
//     const mappedEvent = this.mapMeetingAndSessionsToEvent(meeting, allSessionsForMeeting);
//     const mappedSession = mappedEvent.sessions.find(
//       (session) => session.id === `openf1-session-${targetSession.session_key}`,
//     );

//     if (!mappedSession) {
//       throw new NotFoundException(`Mapped session with id "${id}" not found`);
//     }

//     const drivers = await this.fetchDriversBySessionKey(sessionKey);
//     const laps = await this.fetchLapsBySessionKey(sessionKey);
